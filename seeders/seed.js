...
async function seed() {
  try {
    ...
    if (checkUser.rows.length === 0) {
      ...
      // Update password hash and profile to ensure consistency
      await db.query(`
        UPDATE users SET
          password_hash = $1,
          full_name = $2,
    ...
    if (checkInfo.rows.length === 0) {
      ...
      const sampleTableRows = [
        { komponen: 'Modul Autentikasi & Sesi', pic: 'Luthfi Arviandi', status: 'Selesai', target: '100%', ket: 'Session express & bcrypt' },
        { komponen: 'Binding Data Profil Otomatis', pic: 'Luthfi Arviandi', status: 'Selesai', target: '100%', ket: 'Pre-filled dari database' },
        { komponen: 'Panel Live Preview Infografis', pic: 'Luthfi Arviandi', status: 'Selesai', target: '100%', ket: 'Dynamic DOM rendering' },
        { komponen: 'Ekspor Dokumen Beresolusi Tinggi', pic: 'Luthfi Arviandi', status: 'Selesai', target: '100%', ket: 'Print & PDF ready' }
      ];
